...
**fct_shuffle(f)** is a function for randomly permuting the factor levels. The input of **fct_shuffle** is a factor (character vector). The output is a factor whose factor levels are randomly permuted. Note that **fct_shuffle** preserves the existing "ordered" status of the factor; the output is ordered if the input is ordered, and the output is unordered if the input is not. We can observe how it changes the factor level in the following example.
...
The following examples show the usages of **fct_reorder** for reordering the sequence of boxplots based on median or maximum value.
...
We recode the factor levels of **agegp** with new names ("young", "middle-aged", and "old") in the following example.
...
In the following examples, we keep two levels of **agegp** and replace all other levels with "Other". And then, we replace two levels of **agegp** with "Other" and keep all other levels.
...
**fct_c(...)** is for concatenating factors and combining levels. The input argument **...** are the individual factors that are used for concatenating. **fct_c** can be used to concatenate factors from multiple sources that should have the same levels. In the following example, we concatenate two factors.
...
* **f** is a factor (or character vector).
* **sort** is a logical that determines if we sort the result so that the most common values float to the top.
...
**fct_explicit_na** gives missing value an explicit factor level, ensuring that they appear in summaries and on plots. We can use **fct_explicit_na** when we want to treat all the **NA** as a factor level. In the following example, we see that the new "missing" level is placed at the end of the factor levels.
