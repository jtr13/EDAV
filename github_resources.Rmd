...
If you're working on someone else's repo, make sure you've [configured an upstream remote](https://edav.info/github.html#step-3-configure-remote-that-points-to-the-upstream-repository-once){target="_blank"}.
...
### Undoing stuff
 ...
 - Undo changes in one file since the last commit: `git checkout -- [filename]` [SO link](https://stackoverflow.com/questions/692246/undo-working-copy-modifications-of-one-file-in-git){target="_blank"}

 - Undo deleted branch: Look for the SHA (hash) returned when you deleted the branch. Then `git checkout -b <branch-name> <SHA>` <br> [SO link](https://stackoverflow.com/a/39038276/5314416){target="_blank"}
 
### Random 
 
 - Make local the same as origin: `git fetch origin` <br> `git reset --hard origin/master`   [SO link](https://stackoverflow.com/questions/1628088/reset-local-repository-branch-to-be-just-like-remote-repository-head){target="_blank"}
 
 - Add back a file that was deleted but still exists on another branch `git checkout otherbranch myfile.txt` [SO link](https://stackoverflow.com/questions/307579/how-do-i-copy-a-version-of-a-single-file-from-one-git-branch-to-another){taraget="_blank"}
 
 - Get rid of a file on GitHub that was added to `.gitignore` but is still there: `git rm --cached [filename]` then commit and push changes to GitHub [SO link](https://stackoverflow.com/a/1143800/5314416){target="_blank"}
 
 Slides from Karthik Ram's workshop, [Getting Unstuck from Git](http://inundata.org/lectures/git/#/){target="_blank"}, offer clear ways out of git problems, expressed in English (such as "I royally screwed up the last several commits. I just want life to be good again") , not git speak. An excellent resource!
