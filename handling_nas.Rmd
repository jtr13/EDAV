...
## tl;dr
It's difficult to handle missing data! If your data has some missing values (which it most probably will have) you can either remove such rows, such columns, or impute them.
...
and also see the ratio of number of NAs in each column and row,
```{r}
colMeans(is.na(data))
```
...
These are missing data values which are not related to any missing or non-missing values in other columns in the data.

### Missing at Random (MAR)

These are missing data which are not missing at random, but are linked to one or more groups in the data. The great thing about MAR is that MAR values can be predicted using other features. For eg, people aged greater than 70 generally do not enter their income. 
Most of the data we encounter is MAR.

### Missing Not at Random (MNAR)

Generally, data which is not MAR us MNAR. A big problem is that there is not a huge distinction between MAR and MNAR. We generally assume MAR, unless otherwise.
...
```{r echo = FALSE}
tidy_names <- data %>% gather(key, value, -Name) %>% mutate(missing = ifelse(is.na(value), "yes", "no"))
```
...
And we can also add a scale to check the numerical values available in the dataset to check for some trends,
...
To solve this problem, we can standarize the variables, and even though we would not be able to read the variable-level values, we can 
```{r}
...
Now, we can see the missing trends better! Let us sort them by the number of missing by each row and column,
...
We can also see missing patterns in data by rows,
```{r}
library(mi)
...
We can visualize missing patterns using the visna (VISualize NA) fucntion in the extracat package,
```{r}
extracat::visna(data)
```
...
We can sort the graph by most to least common missing pattern ie, by row, 
```{r}
extracat::visna(data, sort = "r")
```

Or, by most to least missing values, ie by column,
```{r}
...
There are multiple methods to deal with missing values. Some of them are as below:

### Deletion of rows having NAs

These are used when we are handling Missing Completely at Random data.
...
These techniques are generally used with MAR values.
...
Either we can replace all the values in the missing variable directly, for eg, if "Income" has a median of 15000, we replace all the missing values in "Income" with 15000, in a technique known as Generalized Imputation.

Or, we can replace all values on a similar case basis, for eg, we notice that the income of people with Age> 60 is much less than those with Age<60, on an average, and hence we calculate the median Income of each Age group seperately, and impute values separately for each group.

The problem with these methods is that they disturb the underlying distribtion of the data.

### Model Imputation
There are several model based approaches for imputation of data, and several packages, like [mice](https://cran.r-project.org/web/packages/mice/index.html), [Hmisc](https://cran.r-project.org/web/packages/Hmisc/index.html), and [Amelia II](https://cran.r-project.org/web/packages/Amelia/index.html), which deal with this.

You can check out [this](https://datascienceplus.com/imputing-missing-data-with-r-mice-package/) blog on DataScience+ for more information regarding mice and how to use it.
