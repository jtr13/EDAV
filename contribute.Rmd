...
The next step would be [connecting RStudio to Git and Github](https://happygitwithr.com/rstudio-git-github.html). Once connected, we can issue Git commands in the **Terminal** section of RStudio. There shall also be a **Git** tab in RStudio as well.
...
A **local repository** is the repo that resides on your computer. Since we always do work on computers, we need to create a local repository, which is an exact copy of the remote reposiotry.
