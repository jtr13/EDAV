...
<!-- Explanation: -->
Here's a look at the effect of different attributes on each diamond of Fair cut from the "diamonds" dataset:
...
Soemtimes, parallel coordinate plots are very helpful in graphing time series data - where we have information stored at regular time intervals. Each vertical axis will now become a time point and we need to pass that column in ggparcoord's "column" attribute.

## Considerations

### When do I use clustering? 
Generally, when you want to observe a pattern in a set of cases with some specific propreties, you color those cases of interests. That may include diving up all variables into clusters based on their value for a specific categorical vairable. But you can even use a continuous variable - say divide all cases into two sections based on some continuous variable *height*, those who have a height greater than 150cm and those who do not. 
...
### Scales
When we use ggparcoord(), we have an option to set the *scale* attribute, that every value of all variables will be scaled to - so that we can compare their values. 
...
3. uniminmax: scale all values so that the minumum is at 0 and miaximum at 1
...
6. Outlying: order based on the *Outlying* measure in sagnostics
...
## External Resources 
- [Introduction to parallel coordinate plots](https://www.rdocumentation.org/packages/GGally/versions/1.4.0/topics/ggparcoord){target="_blank"}: An excellend resource giving details of all attributes and possible values. Also has some good examples. 
