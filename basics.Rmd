...
* Want a simple description of R Markdown? See [here](https://rmarkdown.rstudio.com/articles_intro.html){target="_blank"} for a description from RStudio on how to combine text, source code, and output into one document.

* Prefer videos? [DataCamp course](https://www.datacamp.com/courses/reporting-with-r-markdown){target="_blank"} to the rescue! We will be using html formatting so focus on that. There is also an [RStudio webinar](https://www.rstudio.com/resources/webinars/getting-started-with-r- markdown/){target="_blank"} about it.
...
(If you're wondering about the difference, `rmarkdown` is the package that converts your text to HTML, while `knitr` runs the R code. In practice the distinction is murky: for example, clicking the "Knit" button in RStudio called the `rmarkdown::render()` function. For more info on what is happening behind the scenes, see [here](https://rmarkdown.rstudio.com/authoring_quick_tour.html){target="_blank"}.
